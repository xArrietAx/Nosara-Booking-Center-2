...
export function ShuttleCard({data}) {
    ...
    const { name, gallery, price, type, distance, demand, duration } = data
    ...
    <div className="relative flex flex-col gap-3 p-7 rounded-tr-[2rem] rounded-tl-[2rem] -mt-10 bg-white">
      <Link href={`/Blog/${data.slug}`} className="link text-lg-bold line-clamp-2" >
        {name}
      </Link>
      ...
      <p className="text-md-medium text-text">/ { type === "Shared" ? "seat" : "ride" }</p>
    </div>
        <Button
          size="sm"
          variant="secondary"
