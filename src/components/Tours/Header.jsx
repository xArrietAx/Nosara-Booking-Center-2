...
export function Header({ page, limit, count, sortBy }) {
  ...
  function handleQuery(key, value, close) {
    ...
    createQueryString(updates, null, null, "tours");
    close();
  }
  ...
  return (
    <div className="flex flex-col items-center justify-between gap-5 pb-5 border-b border-border md:flex-row">
      <div className="flex items-center gap-2 text-text">
        <i className="icon-[mynaui--boat] size-5" />
        <p className="text-sm-bold">
          {start} - {end} of {count} Tours found
        </p>
      </div>
