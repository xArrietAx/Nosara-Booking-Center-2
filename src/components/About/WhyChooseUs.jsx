  ...
  return (
        ...
        <div className="flex flex-col items-center gap-16 lg:flex-row lg:gap-6">
          <ul className="grid flex-1 gap-6 w-full sm:grid-cols-2">
            {content.whyChooseUs.images.map((item, i) => {
              ...
              return (
                <li key={item} className={`relative h-0 p-[50%] rounded-2xl overflow-hidden ${styles[i]}`} >
    ...
    { i === 0 && <VideoModal /> }
          ...
          <div className="flex-1 pt-6 pl-6">
            <BtnInfo variant="secondary" img={content.whyChooseUs.btnInfo.img} >{content.whyChooseUs.btnInfo.text}</BtnInfo>
            <Heading
              as={Tag}
              ...
              {content.whyChooseUs.benefits.map((item, i) => {
                return (
                  <li key={i} className="flex gap-5">
                    <Icon
                      name="GiCheckMark"
                      className="flex-none text-green-500 size-6"
                    />
